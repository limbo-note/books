...
- 跟随者副本
  ...
  - 在过去的10s 内从首领那里获取过最新的消息的副本才能被认为是同步的
  - 不同步的副本可以重新连接zk并获取最新的消息，从而变成同步的
- 首选首领
  - 创建主题时选定的分区首领
...
- 保证分区消息的顺序。保证生产者后写入的消息在消费者处会后读取到
- 消息只有被写入所有同步的副本时（已提交），才能被消费者读取
- 只要有一个副本是活跃的，消息就不会丢失（复制特性带来的可靠性）

一旦一个副本变成了非同步的，它就不会影响消费者的消费速度，但是这意味着同步副本变少（有效复制系数变低），数据丢失风险变大

### 6.3 broker关键配置

#### 复制系数

`replication.factor`（主题级别）、`default.replication.factor`（broker级别，配置自动创建的主题），一般为3

#### 不完全的首领选举

完全的首领选举指的是在重新选举新首领时提交的数据在所有同步的副本上，不完全的则指的是其他副本都是不同步的

这种情况有两个选择（`unclean.leader.election`配置）：

- 不允许不同步的副本成为新首领，等待旧首领恢复
- 允许不同步的副本成为新首领，需要承担丢失数据和数据不一致的风险（可用于对可靠性要求不高的系统）

#### 最少同步副本

`min.insync.replicas`配置，指定最小的同步副本，当同步副本数量小于此数量时，生产者不能写入数据。此时相当于变成了只读的，这也是防止出现数据不一致的情况
...
- 在参数配置和代码里正确处理错误
  - 配置生产者的重试参数
  - 分为可重试错误和不可重试错误（尽量使用Kafka自带的重试机制）
  - 额外的错误处理

### 6.5 可靠地使用消费者

主要在提交偏移量时需要小心。自动提交打开时默认是间隔5s，会在poll()方法中检查是否到时间，若时间到，则提交上一次poll所获取到的偏移量（自动提交适用于数据处理在轮询的循环之内并且不需要维护状态的情况），当数据处理在轮询之外时，最好使用手动提交（同步、异步）

- 总是在处理完数据后再提交偏移量
- 确保提交的偏移量是处理过的最新偏移量而不仅是读取到的最新偏移量
- 再均衡时，在分区被撤销前需要提交偏移量
- 消费者重试（需暂停消费者）、维护状态问题见书
- 数据处理时间较长时，通常放在新的线程中，避免影响poll发送心跳包

### 6.6 验证系统可靠性

#### 配置验证

包括broker配置和客户端配置，Kafka自带的两个类`VerifiableProducer`和`VerifiableConsumer`可用于测试：

- VerifiableProducer生成一系列的消息（从1到某个数字），它会打印发送的日志；VerifiableConsumer类似
- 一般有首d领选举验证、控制器选举验证、broker重启验证、不完全首领选举验证等验证

#### 应用程序验证

包括客户端断开连接、首领选举、重启broker、重启消费者/生产者等

#### 生产环境监控

JMX 度量指标。生产者关注`error-rate`和`retry-rate`，消费者关注`consumer-lag`（即处理速度和分区的最新数据的差距），要保证该值不会呈增大趋势（表示有积压）
